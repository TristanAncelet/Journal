![Arch Linux Logo](ArchLinuxLogo.png)

# Arch Install

This section covers issues/projects that concern my Arch Linux Install, as well as interesting things I read/hear about linux in general.

## Articles in this section

- [Installing Arch](InstallingArch.md)
- [Dual Booting Windows 10](DualBootingWindows10.md)
- [Moving to a new SSD](MovingToNewSSD.md)
- [Mounting Samba Share to Local Directory](MountingSambaShareToLocalDirectory.md)
- [Wifi Connectivity Issues](WifiConnectivityIssues.md)
- [DNS Issues](DNS_Issues.md)
